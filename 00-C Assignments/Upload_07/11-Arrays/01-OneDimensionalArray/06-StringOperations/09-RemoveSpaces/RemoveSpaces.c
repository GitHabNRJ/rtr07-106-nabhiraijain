...
int main(int args[], int* argv[], int* envp[])
{
	...
	for (i = 0; i < nrjisStringLength; i++)
	{
      ...
      switch (nrjchArray_Original[i])
		{
		case 32:

			nrjspace_count++;
			printf("Space index in String array:%d\n", i);
			nrjchArray_Original[i] = '\a';
		  default:
			break;
	...
	printf("\n\n");
	//***********Space and word count is********************/
	printf("The Space count in string Is:");
	printf("%d\n", nrjspace_count);

	printf("The word count in string Is:");
	printf("%d\n", nrjspace_count + 1);
